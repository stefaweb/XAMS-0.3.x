<?php
    ...
    $site_id = gget('siteid');
    if ($site_id)
        array_push($site_ids, array($site_id));
    $sql_add = null;
    $ResellerName = null;
    $errmsg = null;
    ...
    if (!empty($CustomerID))
    {
        array_push($site_ids, $db->db->getCol('SELECT siteid FROM pm_sites_c_customers WHERE customerid = ?', 0, array($CustomerID)));
    ...
    if ($mode === 'search')
    {
        ...
        if (!empty($site_sql)) array_push($site_ids, $db->db->getCol("SELECT id FROM pm_sites s WHERE $site_sql"));
        if (!empty($domain_sql)) array_push($site_ids, $db->db->getCol("SELECT siteid FROM pm_domains d WHERE $domain_sql"));
        if (!empty($user_sql)) array_push($site_ids, $db->db->getCol("SELECT siteid FROM pm_users u WHERE $user_sql"));
        if (strpos($email, '@') > 0)
        {
            ...
            array_push($site_ids, $db->db->getCol($sql, 0, array($domainname, $localpart, $localpart)));
        }

        if (!empty($alias_sql)) array_push($site_ids, $db->db->getCol("SELECT siteid FROM pm_aliases a WHERE $alias_sql"));

        if (!isset($site_ids[0]) || !count($site_ids[0])) $errmsg = $tl->get('Nothing found.');
    }
    ...
    if (!$errmsg)
    {
        ...
        if (isCUSTOMER)
        {
            if (!empty($site_ids_str)) $sql_add = sprintf('AND s.id IN (%s)', addslashes($site_ids_str));
        ...
        elseif (isADMIN)
        {
            ...
            if (!empty($ResellerID)) $sql_add .= 'AND r.id = '. addslashes($ResellerID);
            elseif (!empty($site_ids_str)) $sql_add = addslashes(sprintf(' AND s.id IN (%s)', $site_ids_str));
            $sql = "SELECT      r.name resellername, r.id resellerid, COUNT(rsi.value) raddressbook,
                                r.maxsites ms, r.maxdomains md, r.maxusers mu,
                                r.maxaliases ma, s.id, s.name, s.sitestate, s.maxaddr, s.maxaliases,
                                s.maxquota, s.maxuserquota, s.addrtype, s.viruscheckin, s.viruscheckout,
    ...
    else while ($sr = $sites->fetchRow(DB_FETCHMODE_ASSOC))
    {
        entity_vals($sr);
        $sr['addrtype'] = $db->bitfield2string($sr['addrtype'], 'p,i,s,x');
    
        // Resellers
        if ($ResellerName != $sr['resellername'])
        { // Other reseller as previous iteration?
            if (!empty($ResellerName)) xmlout(1, '</reseller>');
        ...
        if ($aliases->numRows() > 0)
        {
            ...
            while ($ar = $aliases->fetchRow(DB_FETCHMODE_ASSOC))
            {
                ...
                xmlout(5, '<targets>');
                foreach (explode(',', $ar['rightpart']) as $k) tagout(6, 'target', $k);
                xmlout(5, '</targets>');
                xmlout(4, '</alias>');
            }
            xmlout(3, '</aliases>');
    ...
    else
    {
       // header('Content-Type: text/html; charset=UTF-8');
        ...
        $trans = array(
            'patterns' => array('/XHTML Output/', '/XML Output/'),
            'replacements' => array($tl->get('XHTML Output'), $tl->get('XML Output'))
        );
