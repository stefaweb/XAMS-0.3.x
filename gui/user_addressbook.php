<?php
    ...
    if (!empty($button)) {
        $myAB->Assign2Object(['id', 'name', 'ldapname',
        'acl_reseller', 'acl_customer', 'acl_user', 'ord', 'position', ]);

        if ($button == $tl->get('Update')) {
            $i = 0;
            foreach ($myAB->name as $selection) {
                $myAB->Update($i++);
            }
    ...
    {
        global $tl, $overview;
        foreach ([_ACL_NONE => $tl->get('None'), _ACL_READ => $tl->get('Read'), _ACL_WRITE => $tl->get('Read+Write')] as $k=> $elem) {
    ...
    include 'header.php';
?>
<h1><?php echo $tl->get('User Addressbook Administration'); ?></h1>
<?php show_tabs(['Sites'          => 'site_addressbook.php',
                      'Users'     => 'user_addressbook.php',
                      'Aliases'   => 'alias_addressbook.php',
                      'Resellers' => 'reseller_addressbook.php',
                      'Customers' => 'customer_addressbook.php', ], 'Users', $tl) ?>
<p>&nbsp;</p>
<?php
    if (!count($overview)) {
...
<table width="700" class="tbl_global">
    <tr>
        ...
        <th style="font-size: 9pt;"><?php echo $tl->get('ACL Customer') ?></th>
        <th style="font-size: 9pt;"><?php echo $tl->get('ACL User') ?></th>
...
<?php for ($i = 0; $i < count($myAB->overview); $i++) {
            ?> 
    <tr>
        <td class="addressbook">
            <input type="hidden" name="<?php echo "id[$i]" ?>" value="<?php echo $overview[$i]['id'] ?>" />
            <input type="hidden" name="<?php echo "ord[$i]" ?>" value="<?php echo $overview[$i]['ord'] ?>" />
            <input type="text" name="<?php echo "name[$i]" ?>" size="13" maxlength="30" value="<?php echo $overview[$i]['name'] ?>" class="textfield" /></td>
        ...
        <td class="addressbook"><select name="<?php echo "acl_customer[$i]" ?>"><?php echo output_ac($i, 'acl_customer') ?></select></td>
        <td class="addressbook"><select name="<?php echo "acl_user[$i]" ?>"><?php echo output_ac($i, 'acl_user') ?></select></td>
        <td class="addressbook">
            <select name="<?php echo "position[$i]"?>" style="width: 90px;"><option></option><?php for ($i2 = 0; $i2 < count($overview); $i2++) {
                echo '<option value="'.$overview[$i2]['ord'].'">'.$overview[$i2]['name'].'</option>';
            } ?></select>
        </td>
...
<table width="700" class="tbl_global">
    ...
    <tr>
        ...
        <td class="addressbook"><select name="acl_customer"><?php echo output_ac(0, null) ?></select></td>
        <td class="addressbook"><select name="acl_user"><?php echo output_ac(0, null) ?></select></td>
