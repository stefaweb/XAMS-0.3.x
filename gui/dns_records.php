<?php
    ...
    if ($form == 'zone') {
            ...
            if (is_array($rid)) {
                ...
                for ($i = 0; $i < count($rid); $i++) {
                    if (isset($rselected[$i]) && $rselected[$i] == 'true') {
                        $db->execute($sth2, [$rid[$i]]);
                    } else {
                        $db->execute($sth, [$rname[$i], $rtype[$i], $rparameter1[$i], $rparameter2[$i], $rcomment[$i], $rid[$i]]);
                    }
                }
        ...
        $myDNS->Load($dnsid);
        $db->query('INSERT INTO pm_dns_records (dnsid, name, type, parameter1, parameter2, comment) VALUES (?, ?, ?, ?, ?, ?)', [$dnsid, $aname, $atype, $aparameter1, $aparameter2, $acomment]);
        $myDNS->assign('objhaschanged', true);
        $myDNS->Update();
        $myDNS->Load();
    }
...
<?php if ($myDNS->formular_errors) {
    echo '<p class="formerror">'.$tl->get('The formular was not properly filled out. Point at the question mark.').'</p>';
} ?>
            ...
            <tr>
                ...
                <td>
                    <select name="atype">
                        <option></option>
                        <?php
                            foreach (['A', 'AAAA', 'CNAME', 'HINFO', 'MX', 'NS', 'PTR', 'TXT'] as $k) {
            ...
            <tr>
                <td colspan="3">
                    ...
                    <?php
                            ...
                            echo $tl->get('No records for this zone yet. Use the form above to add some.');
                        }
                    ?>
                </td>
            </tr>
