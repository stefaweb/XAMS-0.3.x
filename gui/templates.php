<?php
    ...
    if ($id != null)
    {
        ...
        {
            case 'site':
                if ($button == $tl->get('New')) $url = 'Location: site_template.php?mode=new';
                elseif ($button == $tl->get('Use')) $url = sprintf('Location: site.php?mode=new&templateid=%d', $id);
                elseif ($button == $tl->get('Edit')) $url = sprintf('Location: site_template.php?mode=update&id=%d', $id);
                header($url);
                exit;
            case 'user':
                if ($button == $tl->get('New')) $url = 'Location: user_template.php?mode=new';
                elseif ($button == $tl->get('Use')) $url = sprintf('Location: user.php?mode=new&templateid=%d', $id);
                elseif ($button == $tl->get('Edit')) $url = sprintf('Location: user_template.php?mode=update&id=%d', $id);
                header($url);
                exit;
        }
    }
    ...
    $arr = array(_ADMIN=>'adminid', _RESELLER=>'resellerid', _CUSTOMER=>'customerid');
    $field = $arr[USERT];

    if (USERT != _CUSTOMER)
    {
        $sitet = $db->db->getAll("SELECT id, templatename FROM pm_site_templates WHERE $field = ? ORDER BY templatename", array(USERID), DB_FETCHMODE_ASSOC);
    }

    $usert = $db->db->getAll("SELECT id, templatename FROM pm_user_templates WHERE $field = ? ORDER BY templatename", array(USERID), DB_FETCHMODE_ASSOC);
