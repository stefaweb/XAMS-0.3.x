...
{
    public $objname = 'sites';
    public $notice;
    public $resellers = [];
    public $lngbase = 'site';
    public $tablename = 'pm_sites';

    public function Sites($init = true)
    {
        xclass::xclass($init);
        ...
        if (isCUSTOMER) {
            $this->customers = $this->db->getCol('SELECT customerid FROM pm_sites_c_customers WHERE siteid = ?', 0, [USERID]);
        }
    }
    ...
    public function CalcQuotaUnits($load)
    {
        if ($load) {
            if ($this->maxquota > 0) {
                for ($this->quotaunit = 0; $this->quotaunit < 3; $this->quotaunit++) {
                    if ($this->maxquota % 1024 == 0) {
                        $this->maxquota /= 1024;
                    } else {
                        break;
                    }
            ...
            if ($this->maxuserquota > 0) {
                for ($this->userquotaunit = 0; $this->userquotaunit < 3; $this->userquotaunit++) {
                    if ($this->maxuserquota % 1024 == 0) {
                        $this->maxuserquota /= 1024;
                    } else {
                        break;
                    }
                }
            ...
            if ($this->maxquota > 0) {
                for ($i = 0; $i < $this->quotaunit; $i++) {
                    $this->maxquota *= 1024;
                }
            }
            if ($this->maxuserquota > 0) {
                for ($i = 0; $i < $this->userquotaunit; $i++) {
                    $this->maxuserquota *= 1024;
                }
            }
        }
    ...
    public function add_addressbook()
    {
        ...
        if (count($addressbook_ids) > 0) {
            ...
            $this->db->query('DELETE FROM pm_site_info WHERE siteid = ? AND infofieldid IN (!)', [$this->id, $idstr]);
    ...
    public function Load($id = false)
    {
        ...
        if (empty($this->id)) {
            die($this->objname.'->Load() - ERROR : Have no ID to load!');
        }
    ...
    public function FreeQuota()
    {
        if ($this->maxquota < 0) {
            return -1;
        }
        $sum_maxquota = (int) $this->db->getOne('SELECT SUM(quota) FROM pm_users WHERE quota > 0 AND siteid = ?', [$this->id]);
    ...
    public function FreeUsers()
    {
        ...
        $users = (int) $this->db->getOne('SELECT COUNT(*) FROM pm_users WHERE siteid = ?', [$this->id]);
    ...
    public function FreeAliases()
    {
        ...
        $aliases = (int) $this->db->getOne('SELECT COUNT(*) FROM pm_aliases WHERE siteid = ?', [$this->id]);
    ...
    public function Delete()
    {
        ...
        $ids = [];
        $ids = $this->db->getCol('SELECT id FROM pm_users WHERE siteid = ?', 0, [$this->id]);
        if ($ids) {
            $this->db->query('DELETE FROM pm_user_info WHERE userid IN (!)', $ids);
        }
        ...
        $ids = [];
        $ids = $this->db->getCol('SELECT id FROM pm_aliases WHERE siteid = ?', 0, [$this->id]);
        if ($ids) {
            $this->db->query('DELETE FROM pm_alias_info WHERE aliasid IN (!)', $ids);
        }
