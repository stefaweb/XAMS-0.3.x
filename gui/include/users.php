...
{
    public $objname = 'users';
    public $notice = null;
    public $Name;
    public $lngbase = 'user';
    public $sites = [];
    public $tablename = 'pm_users';

    public function Users($init = true)
    {
        xclass::xclass($init);
        ...
        if (isADMIN) {
            ...
            $this->sites = $this->db->getAll('SELECT   id, name
                                              FROM     pm_sites
                                              WHERE    resellerid = ?
                                              ORDER BY name', [USERID], DB_FETCHMODE_ASSOC);
        } elseif (isCUSTOMER) {
            $this->sites = $this->db->getAll('SELECT    id, name
                                              FROM      pm_sites s
    ...
    public function add_addressbook()
    {
        ...
        if (count($addressbook_ids) > 0) {
            ...
            $this->db->query('DELETE FROM pm_user_info WHERE userid = ? AND infofieldid IN (!)', [$this->id, $idstr]);
    ...
    public function QC_AddUser()
    {
        if (!$this->mySite->id) {
            return 4;
        } elseif ($this->myReseller->FreeUsers() == 0) {
            return 1;
        } elseif ($this->mySite->FreeUsers() == 0) {
            return 2;
        } elseif ($this->mySite->FreeQuota() == 0) {
            return 3;
        } else {
            return 0;
        }
    ...
    public function Add()
    {
        $this->Authenticate();
        if (empty($this->id)) {
            $this->notice = sprintf($this->i18n->get("User '%s' could not be added."), $this->id);
        }
    ...
    $res = $this->db->query('SELECT id, siteid, rightpart FROM pm_aliases WHERE siteid = ?', $this->siteid);
        while ($donnees = mysql_fetch_array($res->_res)) {
            ...
            foreach ($elements as $line) {
                ...
                } elseif (strcmp($line, $this->name) != 0) {
                    $tmp[$i++] = $line;
                    $flag = 1;
                }
            }
