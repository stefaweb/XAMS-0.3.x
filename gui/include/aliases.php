...
{
    public $objname = 'aliases';
    public $notice;
    public $lngbase = 'alias';
    public $tablename = 'pm_aliases';

    public function Aliases($init = true)
    {
    ...
    public function QC_AddAlias()
    {
        if (!$this->mySite->id) {
            return 3;
        } elseif ($this->myReseller->FreeAliases() == 0) {
            return 1;
        } elseif ($this->mySite->FreeAliases() == 0) {
            return 2;
        }
    }
    ...
    public function AliasesOfSite($id)
    {
        $Aliases = $this->db->getOne('SELECT COUNT(*) FROM pm_aliases WHERE siteid = ?', [$id]);
    ...
    public function add_addressbook()
    {
        ...
        if (count($addressbook_ids) > 0) {
            ...
            $this->db->query('DELETE FROM pm_alias_info WHERE aliasid = ? AND infofieldid IN (!)', [$this->id, $idstr]);
