...
{
    public $objname = 'user_addressbook';
    public $notice;
    ...
    public function User_Addressbook()
    {
        xclass::xclass();

        $acl_fields = [null, 'acl_user', 'acl_customer', 'acl_reseller', null];
    ...
    public function Load($userid = false)
    {
        ...
        if ($this->acl_field) {
            $sql = 'SELECT    id, name, value, ! acl
                    ...
                    WHERE     ! > 0
                    ORDER BY  ord';
            $result = $this->db->getAll($sql, [$this->acl_field, $this->userid, $this->acl_field], DB_FETCHMODE_ASSOC);
        } else {
            $sql = 'SELECT    id, name, value
                    ...
                    AND       userid = ?
                    ORDER BY  ord';
            $result = $this->db->getAll($sql, [$this->userid], DB_FETCHMODE_ASSOC);
        }
    ...
    public function Add()
    {
        ...
        $sql = 'INSERT INTO pm_user_info_fields (name, ldapname, acl_reseller, acl_customer, acl_user, ord)
                VALUES (?, ?, ?, ?, ?, ?)';
        $val = [$this->name, $this->ldapname, $this->acl_reseller, $this->acl_customer, $this->acl_user, $max];
        ...
        if ($index >= 0) {
            if (!empty($this->position[$index])) {
                if ($this->position[$index] > $this->ord[$index]) { // Shift field down
                    $this->db->query('UPDATE pm_user_info_fields SET ord = ord-1 WHERE ord <= ? AND ord > ?', [$this->position[$index], $this->ord[$index]]);
                    $this->db->query('UPDATE pm_user_info_fields SET ord = ? WHERE id = ?', [$this->position[$index], $this->id[$index]]);
                } else { // Shift field up
                    $this->db->query('UPDATE pm_user_info_fields SET ord = ord+1 WHERE ord > ? AND ord < ?', [$this->position[$index], $this->ord[$index]]);
                    $this->db->query('UPDATE pm_user_info_fields SET ord = ? WHERE id = ?', [$this->position[$index] + 1, $this->id[$index]]);
                }
            }
            $sql = 'UPDATE pm_user_info_fields
                    SET    name = ?, ldapname = ?, acl_reseller = ?, acl_customer = ?, acl_user = ?
                    WHERE  id = ?';
            $val = [$this->name[$index], $this->ldapname[$index], $this->acl_reseller[$index],
