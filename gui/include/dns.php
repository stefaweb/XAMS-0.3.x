...
class DNS extends xclass
{
    var $objname = 'dns';
    var $notice;
    var $multiplier = array(1,60,3600,86400,604800,2592000);
    var $units = array('ttl', 'refresh', 'retry', 'expire', 'nttl');
    var $records = array();
    var $lngbase = 'dns';
    var $tablename = 'pm_dns';

    function DNS($init=true)
    {
        xclass::xclass($init);
        $this->CalcUnits();
    }
    ...
    function Load($id=false)
    {
        ...
        $this->records = $this->db->getAll('SELECT id, name, type, parameter1, parameter2, comment FROM pm_dns_records WHERE dnsid = ? ORDER BY type, name', array($this->id), DB_FETCHMODE_ASSOC);
    ...
    {
        $this->multiply();
        if ($this->ObjectChanged()) $this->CalculateSerial();
