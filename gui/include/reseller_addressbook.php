...
{
    public $objname = 'reseller_addressbook';
    public $notice;
    ...
    public function Reseller_Addressbook($init = true)
    {
        xclass::xclass($init);
    ...
    public function Load($resellerid = false)
    {
        if ($resellerid) {
    ...
    public function Add()
    {
        ...
        $val = [$this->name, $this->ldapname, $this->acl_reseller, $max];
        ...
        if ($index >= 0) {
            if (!empty($this->position[$index])) {
                if ($this->position[$index] > $this->ord[$index]) { // Shift field down
                    $this->db->query('UPDATE pm_reseller_info_fields SET ord = ord-1 WHERE ord <= ? AND ord > ?', [$this->position[$index], $this->ord[$index]]);
                    $this->db->query('UPDATE pm_reseller_info_fields SET ord = ? WHERE id = ?', [$this->position[$index], $this->id[$index]]);
                } else { // Shift field up
                    $this->db->query('UPDATE pm_reseller_info_fields SET ord = ord+1 WHERE ord > ? AND ord < ?', [$this->position[$index], $this->ord[$index]]);
                    $this->db->query('UPDATE pm_reseller_info_fields SET ord = ? WHERE id = ?', [$this->position[$index] + 1, $this->id[$index]]);
                }
            }
            $sql = 'UPDATE pm_reseller_info_fields
                    SET    name = ?, ldapname = ?, acl_reseller = ?
                    WHERE  id = ?';
            $val = [$this->name[$index], $this->ldapname[$index], $this->acl_reseller[$index], $this->id[$index]];
