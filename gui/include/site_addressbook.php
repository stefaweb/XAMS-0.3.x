...
{
    public $objname = 'site_addressbook';
    public $notice;
    ...
    public function Site_Addressbook($init = true)
    {
        xclass::xclass($init);
    ...
    public function Load($siteid = false)
    {
        if ($siteid) {
            $this->siteid = $siteid;
        }
        ...
        if ($index >= 0) {
            if (!empty($this->position[$index])) {
                if ($this->position[$index] > $this->ord[$index]) { // Shift field down
                    $this->db->query('UPDATE pm_site_info_fields SET ord = ord-1 WHERE ord <= ? AND ord > ?', [$this->position[$index], $this->ord[$index]]);
                    $this->db->query('UPDATE pm_site_info_fields SET ord = ? WHERE id = ?', [$this->position[$index], $this->id[$index]]);
                } else { // Shift field up
                    $this->db->query('UPDATE pm_site_info_fields SET ord = ord+1 WHERE ord > ? AND ord < ?', [$this->position[$index], $this->ord[$index]]);
                    $this->db->query('UPDATE pm_site_info_fields SET ord = ? WHERE id = ?', [$this->position[$index] + 1, $this->id[$index]]);
                }
            }
