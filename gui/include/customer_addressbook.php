...
{
    var $objname = 'customer_addressbook';
    var $notice;
    ...
    function Customer_Addressbook($init=true)
    {
        xclass::xclass($init);
    ...
    function Load($customerid=false)
    {
    ...
    function Update($index = -1)
    {
        if ($index >= 0)
        {
            if (!empty($this->position[$index]))
            {
                ...
                { // Shift field down
                    $this->db->query('UPDATE pm_customer_info_fields SET ord = ord-1 WHERE ord <= ? AND ord > ?', array($this->position[$index], $this->ord[$index]));
                    $this->db->query('UPDATE pm_customer_info_fields SET ord = ? WHERE id = ?', array($this->position[$index], $this->id[$index]));
                }
                else
                { // Shift field up
                    $this->db->query('UPDATE pm_customer_info_fields SET ord = ord+1 WHERE ord > ? AND ord < ?', array($this->position[$index], $this->ord[$index]));
                    $this->db->query('UPDATE pm_customer_info_fields SET ord = ? WHERE id = ?', array($this->position[$index]+1, $this->id[$index]));
                }
            }
