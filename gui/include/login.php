...
class Login extends xclass
{
    ...
    function Login($init=true)
    {
        xclass::xclass($init);
    }

    function doLogin($username, $password, $usertype = null)
    {
        ...
        {
            case 'admin':
                if (_ADMIN_LOGIN) $LoginOK = $this->DoAdminLogin();
                break;
            case 'reseller':
                if (_RESELLER_LOGIN) $LoginOK = $this->DoResellerLogin();
                break;
            case 'customer':
                if (_CUSTOMER_LOGIN) $LoginOK = $this->DoCustomerLogin();
                break;
            case 'user':
                if (_USER_LOGIN) $LoginOK = $this->DoUserLogin();
                break;
        }
    ...
    {
        $LoginOK = false;
        if (strpos($this->login, '@') === false) return false;
        ...
        $sql = 'SELECT      u.id, u.password, u.AddrType
                ...
                WHERE       d.name = ?
                AND         u.name = ?';
        $result = $this->db->getRow($sql, array($domain, $name));
    ...
    function DoCustomerLogin()
    {
        ...
        $sql = 'SELECT id, password, locked, failures
                FROM   pm_customers
                WHERE  name = ?';
        $result = $this->db->getRow($sql, array($this->login));
