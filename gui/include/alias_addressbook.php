...
class Alias_Addressbook extends xclass
{
    ...
    function Alias_Addressbook($init=true)
    {
        xclass::xclass($init);

        $acl_fields = array(null, null, 'acl_customer', 'acl_reseller', null);
    ...
    function Load($aliasid=false)
    {
        if ($aliasid) $this->aliasid = $aliasid;
        ...
        if ($this->acl_field)
        {
            $sql = 'SELECT    id, name, value, ! acl
                    ...
                    WHERE     ! > 0
                    ORDER BY  ord';
            $result = $this->db->getAll($sql, array($this->acl_field, $this->aliasid, $this->acl_field), DB_FETCHMODE_ASSOC);
        }
        else
        {
            $sql = 'SELECT    id, name, value
                    ...
                    AND       aliasid = ?
                    ORDER BY  ord';
            $result = $this->db->getAll($sql, array($this->aliasid), DB_FETCHMODE_ASSOC);
        }
    ...
    function Update($index = -1)
    {
        if ($index >= 0)
        {
            if (!empty($this->position[$index]))
            {
                ...
                { // Shift field down
                    $this->db->query('UPDATE pm_alias_info_fields SET ord = ord-1 WHERE ord <= ? AND ord > ?', array($this->position[$index], $this->ord[$index]));
                    $this->db->query('UPDATE pm_alias_info_fields SET ord = ? WHERE id = ?', array($this->position[$index], $this->id[$index]));
                }
                else
                { // Shift field up
                    $this->db->query('UPDATE pm_alias_info_fields SET ord = ord+1 WHERE ord > ? AND ord < ?', array($this->position[$index], $this->ord[$index]));
                    $this->db->query('UPDATE pm_alias_info_fields SET ord = ? WHERE id = ?', array($this->position[$index]+1, $this->id[$index]));
                }
            }
