<?php
    ...
    $user_arr = array(_USER=>'USER', _CUSTOMER=>'CUSTOMER', _RESELLER=>'RESELLER', _ADMIN=>'ADMIN');
    ...
    {
        if ($mode)
            if (USERT != $usertype) die('Insufficient access-rights!');
        else
            if (USERT < $usertype) die('Insufficient access-rights!');
        if (USERT < _USER || USERT > _ADMIN) die('What the hell are you?!');
    }
?>