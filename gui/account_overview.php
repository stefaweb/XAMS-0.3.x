<?php
    ...
    switch (gpost('form_type')) {
        case 'admin':
            if ($button == $tl->get('New')) {
                $url = 'Location: administrator.php?mode=new';
            } elseif ($button == $tl->get('Edit')) {
                $url = sprintf('Location: administrator.php?mode=update&id=%d', $AdminID);
            }
            header($url);
            exit;
        case 'reseller':
            if ($button == $tl->get('New')) {
                $url = 'Location: reseller.php?mode=new';
            } elseif ($button == $tl->get('Edit')) {
                $url = sprintf('Location: reseller.php?mode=update&id=%d', $ResellerID);
            } elseif ($button == $tl->get('Overview')) {
                $url = sprintf('Location: system_overview.php?resellerid=%d', $ResellerID);
            }
            header($url);
            exit;
        case 'customer':
            if ($button == $tl->get('New')) {
                $url = 'Location: customer.php?mode=new';
            } elseif ($button == $tl->get('Edit')) {
                $url = sprintf('Location: customer.php?mode=update&id=%d', $CustomerID);
            } elseif ($button == $tl->get('Overview')) {
                $url = sprintf('Location: system_overview.php?customerid=%d', $CustomerID);
            }
            header($url);
            exit;
    }
    ...
    if (isADMIN) {
        ...
        $result_customer = $db->db->getAll('SELECT id, name, locked FROM pm_customers WHERE resellerid = ? ORDER BY name', [USERID], DB_FETCHMODE_ASSOC);
    }
