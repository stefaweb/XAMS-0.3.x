<?php
    ...
    if (!$MEPP) $MEPP = $myPREFS->loglines; // Max. entries per page
    ...
    if ($delete != null)
    {
        ...
        foreach(array('pm_log_message', 'pm_logs_c_admins', 'pm_logs_c_resellers', 'pm_logs_c_customers', 'pm_logs_c_users') 
            as $tbl_name)
        {
    ...
    if ($filter != null)
    {
        ...
        if ($usertype) $sql .= " AND $usertype > 0";
        if ($username) $sql .= " AND (name LIKE '$username')";
        if ($resource) $sql .= " AND resource = '$resource'";
        if ($type) $sql .= " AND msgtype = '$type'";
        if ($status) $sql .= " AND msgstatus = '$status'";
    ...
    if ($button)
    {
        ...
        if ($page-1 != $current_page) $start_pos=$MEPP*($current_page = $page-1);
        switch ($button)
        {
    ...
    if (!isset($start_pos)) $start_pos = 0;

    $sql .= "\nORDER BY TIMESTAMP DESC
             LIMIT $start_pos,$MEPP";

    $result = $db->query($sql);
    $UserTypes_colors = array('adminid' => null, 'resellerid' => '#FADCA7', 'customerid' => '#DADCA7', 'userid' => '#C1E1D7');
            ...
            <tr>
                ...
                <td>
                    <select name="resource">
                        <?php foreach (array('', 'XAMS', 'SMTP', 'POP', 'IMAP', 'PAM', 'FTP') as $elem) { ?>
                        <option<?php if ($resource == $elem) echo ' selected="selected"'?>><?php echo $elem?></option>
                        <?php } ?>
                    </select>
                </td>
		<td>
            ...
            <tr>
                ...
                <td>
                    <select name="usertype">
                        <option />
                        <option value="adminid"<?php if ($usertype == "adminid") echo ' selected="selected"'?>><?php echo $tl->get("Admin")?></option>
                        <option value="resellerid"<?php if ($usertype == "resellerid") echo ' selected="selected"'?>><?php echo $tl->get("Reseller")?></option>
                        <option value="customerid"<?php if ($usertype == "customerid") echo ' selected="selected"'?>><?php echo $tl->get("Customer")?></option>
                        <option value="userid"<?php if ($usertype == "userid") echo ' selected="selected"'?>><?php echo $tl->get("User")?></option>
                    </select>
                </td>
                ...
                <td>
                    <select name="type">
                        <option />
                        <option value="Login"<?php if ($type == "Login") echo ' selected="selected"'?>><?php echo $tl->get("Login")?></option>
                        <option value="Selection"<?php if ($type == "Selection") echo ' selected="selected"'?>><?php echo $tl->get("Selection")?></option>
                        <option value="Update"<?php if ($type == "Update") echo ' selected="selected"'?>><?php echo $tl->get("Update")?></option>
                        <option value="Insertion"<?php if ($type == "Insertion") echo ' selected="selected"'?>><?php echo $tl->get("Insertion")?></option>
                        <option value="Deletion"<?php if ($type == "Deletion") echo ' selected="selected"'?>><?php echo $tl->get("Deletion")?></option>
                    </select>
                </td>
				<td>
            ...
            <tr>
                ...
                <td>
                    <select name="status">
                        <option />
                        <option value="ok"<?php if ($status == "ok") echo ' selected="selected"'?>><?php echo $tl->get("OK")?></option>
                        <option value="failed"<?php if ($status == "failed") echo ' selected="selected"'?>><?php echo $tl->get("FAILED")?></option>
                    </select>
                </td>
				<td>
...
<?php if ($result->numRows()) { ?>
    ...
    <div style="width: 710px"><hr /></div>
    <p>
        <input type="submit" name="button" value="|&lt;" class="button"<?php if ($current_page == 0) echo ' disabled="disabled"'?> />
        <input type="submit" name="button" value="&lt;" class="button"<?php if ($current_page == 0) echo ' disabled="disabled"'?> />
        <input type="submit" name="button" value="&gt;" class="button"<?php if ($current_page+1 == $pages) echo ' disabled="disabled"'?> />
        <input type="submit" name="button" value="&gt;|" class="button"<?php if ($current_page+1 == $pages) echo ' disabled="disabled"'?> />
...
<?php
    ...
    {
        for ($i=1; $i<=$indent; $i++) $GLOBALS['xmldata'] .= "\t";
        $GLOBALS['xmldata'] .= $str;
        $GLOBALS['xmldata'] .= "\n";
    }
    ...
    {
        foreach ($vals as $k=>$elem) $vals[$k] = htmlspecialchars($elem, ENT_COMPAT, 'UTF-8');
    }
...
<?php } else echo $tl->get('No log entries found') ?>
